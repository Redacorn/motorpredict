...
def arg_parse():
    ...
    parser.add_argument('--data_path', type=str, default='/home/gpuadmin/test_data/transformed/vibration', help='data path')
...
def df_preprocess(datapath):
    ...
    for file in datapath:
        ...
        if state(file) == 0:
            data['state'] = 0
            df_0 = pd.concat([df_0, data])
        elif state(file) == 1:
            data['state'] = 1
            df_1 = pd.concat([df_1, data])
        elif state(file) == 2:
            data['state'] = 2
            df_2 = pd.concat([df_2, data])
        elif state(file) == 3:
            data['state'] = 3
